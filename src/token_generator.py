...
class TokenGenerator:
    ...
    def get_matching_word(self, sub_str: str, prompt: str, arg_type: str) -> str | None:
    ...
    def generate_args_val(self, allowed_tokens: List[int], arg_name: str,
        ...
        while token != terminating_token[0] and len(complete_arg_tokens) < self.tkn_limits:
            ...
            if arg_type == "float" or arg_type == "int" or arg_type == "number":
                ...
                sub_str += str_val
                matching_word = self.get_matching_word(sub_str, prompt, arg_type)
                ...
                token = self.get_next_str_token(logits, set(allowed_tokens), arg_name, token_counter, soft_bias)
                str_val = self.decode(token)
                sub_str += str_val
                matching_word = self.get_matching_word(sub_str, prompt, arg_type)
                if matching_word is not None:
    ...
    def get_next_str_token(self, logits: List[float],
        ...
        if arg_name == 'replacement' and counter == 1:
            symbol_pattern = {'*', '#', ' ', '_', '$', '@'}
    ...
    def show_top_logits(self, logits_np: List[int], max_logits_idx: int, top: int) -> None:
        ...
        for token in top_ten:
            ...
            tokens_with_prob += f"{self.decode([token])}({round(logits_np[token], 2)}),"
        tokens_with_prob += f"\033[92mSelected token: {self.decode([max_logits_idx])}\033[0m"
