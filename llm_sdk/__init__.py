...
# ABOUTME: Provides Small_LLM_Model class for
# loading and running causal language models.
...
class Small_LLM_Model:
    """Utility class wrapping a lightweight Hugging Face
     causal-LM for fast, low-memory experimentation.
    ...
    dtype: torch.dtype | None, default=None
        ...
        usage reasonable; on CPU we keep ``float32`` for
    ...
    def __init__(
        ...
        *,
        ...
        if dtype is None:
            dtype = torch.float16 \
                if self._device in ["cuda", "mps"] else torch.float32
        self._dtype = dtype
